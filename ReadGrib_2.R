ReadGrib_2 <- function(file.names, levels, variables, domain = NULL, domain.type = "latlon", file.type = "grib2", missing.data = NULL) {
  ...
  if(file.type == "grib2") {
    ...
    # DEFINE NEW FUNCTION TO EXTRACT A SINGLE GRIB2 FILE
    #------------------------------------------------------------------------------------------------------
    
      oneFileExtract<-function(file.name,variables,levels,domain,domain.type = "latlon",missing.data.str,match.str){
      ...
      } else { return(model.data.vector)}
    }
      
      # VECTORIZE FUNCTION
      #------------------------------------------------------------------------------------------------------
      vecReadGribs<-Vectorize(oneFileExtract,"file.name")
      
      # CALL VECTORIZED FUNCTION
      #------------------------------------------------------------------------------------------------------
      model.data.vector<-as.vector(vecReadGribs(file.names,variables=variables,levels=levels,domain=domain,
                                missing.data.str=missing.data.str,match.str=match.str))
      
    
        #Report data
        chunk.inds <- seq(1, length(model.data.vector) - 6, by = 7)
        model.run.date <- model.data.vector[chunk.inds]
        forecast.date  <- model.data.vector[chunk.inds+1]
        variables.tmp  <- model.data.vector[chunk.inds + 2]
        levels.tmp     <- model.data.vector[chunk.inds + 3]
